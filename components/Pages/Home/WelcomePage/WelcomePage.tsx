...
function ParticleCanvas() {
  ...
  useEffect(() => {
    ...
    const pts = Array.from({ length: 28 }, () => ({
      ...
      r: Math.random() * 3.5 + 1.2, vx: (Math.random() - 0.5) * 0.5,
      vy: Math.random() * 0.35 + 0.12, o: Math.random() * 0.3 + 0.45,
      spin: Math.random() * 0.03 - 0.015, angle: Math.random() * Math.PI * 2,
    }));
    let raf: number;
    const draw = () => {
...
function BuildMeter() {
  ...
  useEffect(() => { const t = setTimeout(() => setFill(94), 750); return () => clearTimeout(t); }, []);
  return (
    <div className={styles.thermo} aria-hidden="true">
      ...
      <div className={styles.thermoLabels}>
        ...
        <span className={styles.thermoBot}>Draft</span>
      </div>
    </div>
  );
}

const CHIPS = ['Free Estimates', '3-Yr Warranty', 'Licensed & Insured', '12+ Yrs Local', 'HOA-Friendly'];
      ...
      <div className={styles.layout}>
        ...
        <div className={styles.content}>
          <motion.div className={styles.badge}
            ...
            Waco&apos;s Fence &amp; Deck Builder — Since 2012
          </motion.div>

          <motion.h1 className={styles.headline}
            initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, delay: 0.1 }}>
            Frame It.<br />Finish It.<br />
            <span className={styles.accentLine}>TimberLine.</span>
          </motion.h1>

          <motion.p className={styles.sub}
            initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.55, delay: 0.22 }}>
            Fences &amp; decks built to last — Central Texas quality. Flat-rate quotes,
            licensed crews, and a 3-year workmanship warranty on every install.
          </motion.p>

          <motion.div className={styles.ctaRow}
            initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5, delay: 0.34 }}>
            <a href="tel:+12547536400" className={styles.ctaPrimary}>
        ...
        >
          ...
          <motion.div className={`${styles.statCard} ${styles.sc1}`}
            ...
            <span className={styles.scNum}>3,400+</span>
            <span className={styles.scLbl}>Fences &amp; Decks</span>
          </motion.div>
          ...
          <motion.div className={`${styles.statCard} ${styles.sc3}`}
            ...
            <span className={styles.scNum}>3-Year</span>
